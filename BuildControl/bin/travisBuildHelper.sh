...
while [[ $THIS_TRY < $MAXIMUM_TRIES ]]; do
	...
	( set -o pipefail && xcodebuild -project CleanroomText.xcodeproj -configuration Release ONLY_ACTIVE_ARCH=YES -scheme CleanroomText-$PLATFORM -destination "$DESTINATION" -destination-timeout 300 $XCODE_ACTION 2>&1 | tee "CleanroomText-$PLATFORM-$OPERATION.log" | xcpretty )
