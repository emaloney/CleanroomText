...
xcodebuild -project CleanroomText.xcodeproj -configuration Debug -scheme CleanroomText -destination 'platform=iOS Simulator,OS=10.3,name=iPad Air 2' -destination-timeout 300 clean build
...
# 	fi
# 	
# 	( set -o pipefail && xcodebuild -project CleanroomText.xcodeproj -configuration Debug -scheme "CleanroomText" -destination "$DESTINATION" -destination-timeout 300 $XCODE_ACTION 2>&1 | tee "CleanroomText-$PLATFORM-$OPERATION.log" | xcpretty )
# 	XCODE_RESULT="${PIPESTATUS[0]}"
# 	if [[ "$XCODE_RESULT" == "0" ]]; then
# 		rm "CleanroomText-$PLATFORM-$OPERATION.log"
# 		exit 0
