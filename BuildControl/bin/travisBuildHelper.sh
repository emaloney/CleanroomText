...
( set -o pipefail && xcodebuild -project CleanroomText.xcodeproj -configuration Debug -scheme "CleanroomText" -destination "$DESTINATION" -destination-timeout 300 $BUILD_ACTION 2>&1 | tee "CleanroomText-$PLATFORM-$OPERATION.log" | xcpretty )
