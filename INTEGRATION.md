...
*Integration* is the act of embedding the `CleanroomText.framework` binary into your project, thereby exposing the API it provides to your code.

CleanroomText is built as a *Swift framework*, and as such, it has the following base platform requirements:
...
**This is the `swift2.3` branch of CleanroomText.** It uses **Swift 2.3** and **requires Xcode 8.0 beta 6** (or higher) to compile.
...
- **[Carthage integration](#carthage-integration)** — Explains how to use the [Carthage](https://github.com/Carthage/Carthage) dependency manager for adding CleanroomText to your project.

- **[Manual integration](#manual-integration)** — Demonstrates the steps for adding CleanroomText by embedding `CleanroomText.xcodeproj` within your own Xcode project. (**Note:** Manual integration is a bit more involved than using Carthage).
...
When building a framework for a platform with a simulator, Carthage creates a *universal binary*, allowing `CleanroomText.framework` to work in the simulator as well as on actual devices.
...
1. Update the `Cartfile` with an entry for CleanroomText
2. Download and build CleanroomText
3. Add `CleanroomText.framework` to your application target
...
```
github "emaloney/CleanroomText"
```

### 2. Download & Build using Carthage

The command `carthage update` causes Carthage to download and build CleanroomText.

By default, `carthage update` builds all platform targets in a project. Normally, this is _not_ what you want; you’ll usually use CleanroomText on just one platform at a time.
...
Once Carthage is done building CleanroomText, you can use the `open` command to locate the framework binaries in Finder:
...
If all went well, the Carthage build directories should contain the file `CleanroomText.framework`. If that file isn’t present, something went wrong with the build.
...
Go back to Finder, select the file `CleanroomText.framework`, and then drag it into the list area directly below *Embedded Binaries*.

If successful, you should see `CleanroomText.framework` listed under both the *Embedded Binaries* and *Linked Frameworks and Libraries* sections.
...
```
"$PROJECT_DIR"/Carthage/Checkouts/CleanroomText/BuildControl/bin/stripCarthageFrameworks.sh
```
...
Once you’ve done this, try building your application. If you don’t see any errors, **_you’re all done integrating CleanroomText!_**

But before you start coding, skip to the [Adding the Swift import](#adding-the-swift-import) section to see how you can import CleanroomText for use in your Swift code.

## Manual Integration

Manual integration involves embedding `CleanroomText.xcodeproj` directly in your Xcode project.

This will ensure that CleanroomText is built with the exact same settings you’re using for your app. You won’t have to fiddle with different settings for different architectures — when you’re running in the simulator, it will work; when you then switch to building for device, it’ll work, too.

You’ll also be able to step into CleanroomText code directly in the debugger without worrying about `.dSYM` resolution, which is very helpful.

### An Overview of the Process

Manual integration is a bit involved; there are five high-level tasks that you’ll need to perform:

1. Download the CleanroomText source into your project structure
2. Embed `CleanroomText.xcodeproj` in your Xcode project
3. Build `CleanroomText.framework`
4. Add `CleanroomText.framework` to your application target
...
For our integration examples, we’re going to be showing the top-level `CleanroomText` directory inside a `Libraries` directory at the root level of of your application’s source.
...
### 1. Download the CleanroomText source

If you’re already using git for version control, we recommend adding CleanroomText to your project as a submodule. This will allow you to “lock” your codebase to specific versions of CleanroomText, making it easier to incorporate new versions on whatever schedule works best for you.

If you’re using some other form of version control of if you’re not using version control at all—*shame on you!*—then you’ll want to *clone* the CleanroomText repository. We suggest putting the CleanroomText clone somewhere within your application’s directory structure, so that it is included in whatever version control regimen you’re using.

#### Downloading CleanroomText as a submodule

> **Important:** Skip this section if you plan to download CleanroomText using `git clone`.

From within the `Libraries` directory, issue the following commands to download CleanroomText:

```bash
git submodule add https://github.com/emaloney/CleanroomText
git submodule update --init --recursive
```

Next, you’re ready to [embed the `CleanroomText.xcodeproj` project file in your Xcode project](#2-embed-cleanroomtext-in-your-project).

#### Downloading CleanroomText as a cloned repo

> **Important:** Skip this section if you already performed the tasks outlined in “Downloading CleanroomText as a submodule” above.

From within the `Libraries` directory, issue the following command to clone the CleanroomText repository:

```bash
git clone https://github.com/emaloney/CleanroomText
```

### 2. Embed CleanroomText in your project

In the Terminal, the command `open CleanroomText` to open the folder containing the CleanroomText source in the Finder. This will reveal the `CleanroomText.xcodeproj` Xcode project and all files needed to build `CleanroomText.framework`.

Then, open your application in Xcode, and drag `CleanroomText.xcodeproj` into the Xcode project browser. This will embed CleanroomText in your project and allow you to add the targets built by CleanroomText to your project.

### 3. Build CleanroomText

Before we can add `CleanroomText.framework` to your app, we have to build it, so Xcode has more information about the framework.
...
Once a device-based run destination has been selected, select the appropriate build scheme for the target platform: “CleanroomText-iOS”, “CleanroomText-OSX”, “CleanroomText-tvOS” or “CleanroomText-watchOS”.

Then, select *Build* (⌘B) from the *Product* menu.

Once the build is complete, open `CleanroomText.xcodeproj` in the project navigator and find the “Products” group. Open that, and right-click on `CleanroomText.framework`. Select *Show in Finder*. This will open the folder containing the framework binary you just built.

You may see several files in this folder; the one we’re concerned with is:

- `CleanroomText.framework`
...
So, to make things sane again, you’ll need to make sure Xcode references `CleanroomText.framework` using a “Relative to Build Products” location.

To do this:

1. Locate `CleanroomText.framework` in the Xcode project browser
...
Once you’ve done this for each framework, **_you’re all done integrating CleanroomText!_**

## Adding the Swift import

Once CleanroomText has been successfully integrated, all you will need to do is add the following `import` statement to any Swift source file where you want to use CleanroomText:

```swift
import CleanroomText
```

Want to learn more about CleanroomText? Check out [the README](https://github.com/emaloney/CleanroomText/blob/master/README.md).
