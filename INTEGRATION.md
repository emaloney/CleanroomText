...
*Integration* is the act of embedding the `CleanroomText.framework` binary into your project and exposing the API it provides to your code.

Note that CleanroomText is built as a *Swift framework*. This has several implications, not the least of which is that it will only work on iOS 8 or above. It is also only supported for use by other Swift code. Some, all or none of it may work from Objective-C; we haven't tried it, we wouldn't recommend it, and we don't support it.
...
CleanroomText requires a **mimimum Xcode version of 6.3** to be built, and the resulting binary can be used on **iOS 8.1 and higher**.

We'll also be using the `git` command in the Terminal for installing the CleanroomText repo in your codebase. These steps have been tested with **git 2.3.2 (Apple Git-55)**, although they should be compatible with a wide range of git versions.
...
- **[Manual integration](#manual-integration)** — The `CleanroomText.xcodeproj` Xcode project file is embedded directly within your project. You then add `CleanroomText.framework` to the *Embedded Binaries* and *Linked Frameworks and Libraries* sections under the *General* tab for your application target.

- **[Carthage integration](#carthage-integration)** — [Carthage](https://github.com/Carthage/Carthage) is a dependency package manager designed to build frameworks. Once Carthage is installed, to add CleanroomText to your project using Carthage, you would put the line `github "emaloney/CleanroomText"` in your `Cartfile` and then issue the command `carthage update`.
...
From within the `Libraries` directory, issue the following command to download CleanroomText:

```bash
git submodule add https://github.com/emaloney/CleanroomText.git
```

Next, you're ready to [embed the `CleanroomText.xcodeproj` project file in your Xcode project](#2-embed-cleanroomasl-in-your-project).
...
Once a device-based run destination has been selected, select the "CleanroomText" build scheme. Then, select *Build* (⌘B) from the *Product* menu.
...
Go back to Finder and drag `CleanroomText.framework` into the list area directly below *Embedded Binaries* in Xcode.
...
Once you've done this, **_you're all done integrating CleanroomText!_**

Skip to the [Adding the Swift import](#adding-the-swift-import) section to see how you can import CleanroomText for use in your Swift code.
...
This will cause Carthage to download and build CleanroomText.
...
Once Carthage is done building CleanroomText and its dependencies, you can execute the following Terminal command to see the frameworks built by Carthage:

```bash
open Carthage/Build/iOS
```

This will cause the directory containing `CleanroomText.framework` to open in Finder.
...
Want to read more about CleanroomText? Check out [the README](https://github.com/emaloney/CleanroomText/blob/master/README.md).
